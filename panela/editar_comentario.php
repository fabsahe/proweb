<?php 
  ...
  if( !isset($_GET['id']) ) {
    ...
    $id = $_GET['id'];
    $consulta = $con->query("SELECT * FROM comentarios WHERE id=$id");
        ...
        <div class="container-fluid">
          ...
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">Info del comentario</h6>
            </div>
            <div class="card-body">
              <div class="col-lg-6">

              <form enctype="multipart/form-data" method="post" action="consultas/edit_com.php">
                <div class="form-group">
                  <input type="hidden" name="id" value="<?=$id?>">
                  <label for="autor">Autor</label>
                  <input type="text" class="form-control form-control-user" id="autor" name="autor" value="<?=$origin['autor']?>" required>
                </div>
                <div class="form-group">
                  <label for="contenido">Contenido</label>
                  <textarea class="form-control form-control-user" id="contenido" name="contenido" required="required"><?=$origin['contenido']?> </textarea>
                </div>        
                <hr>       
